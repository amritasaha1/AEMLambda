		...
		try {
			...
			if (!(thanks.equalsIgnoreCase("null"))) {
				...
				if (!(filltypeintent.equalsIgnoreCase("null"))) {
					...
					String formLinkMessage = "Here is the link to a partially filled application for you – "
							+ defaultLink;
					...
					if (!(usercardintent.equalsIgnoreCase("null"))) {
						...
						if (!(featureIntent.equalsIgnoreCase("null"))) {
							if (featureIntent.equalsIgnoreCase("cash rewards")
									|| featureIntent.equalsIgnoreCase("travel miles")
									|| featureIntent.equalsIgnoreCase("hotel miles")
									|| featureIntent.equalsIgnoreCase("nothing")
									|| featureIntent.equalsIgnoreCase("no")) {
								LambdaLogger logger = context.getLogger();
								...
								dialogAction = new DialogAction("ElicitSlot", "FirstCreditIntent", slots,
										...
										new Message("PlainText", "Here are some good offers for you - What type of card would you like to choose?"));
								i = i + 1;
							} else {
								dialogAction = new DialogAction("Close", "Fulfilled", new Message("PlainText",
										"Sorry, I did not get you; Please try again from beginning. You have a nice day!"));
							}
						} else {
							// Score
			...
			dialogAction = new DialogAction("Close", "Fulfilled",
					new Message("PlainText", "Sorry , I did not understand. Please try again from beginning."));
			return new LexResponse(dialogAction);
		}
...
//		}
		...
		Button button = null;
		button = new Button("Gold Card","Gold Card");
		buttonArray[0] = button;
		button = new Button("Silver Card","Silver Card");
		buttonArray[1] = button;
		button = new Button("Platinum Card","Platinum Card");
		buttonArray[2] = button;
